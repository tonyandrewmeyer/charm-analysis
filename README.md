...
Clean up disk space used by cached charm repositories. The `.cache/` folder can
grow to 20GB+ due to build artifacts generated by running tools like tox.

```shellscript
$ python clean_cache.py --help
Usage: clean_cache.py [OPTIONS]
...
Options:
  ...
  --full               Remove the entire cache folder.
  --dry-run            Show what would be removed without deleting.
  --help               Show this message and exit.
```

By default (light clean), removes known junk directories from cached repos:
`.tox/`, `.mypy_cache/`, `__pycache__/`, `.pytest_cache/`, `.ruff_cache/`,
`.coverage`, `*.egg-info/`, and runs `git clean -fdx` in each repo to catch
other untracked build artifacts.

Use `--full` to delete the entire cache folder, and `--dry-run` to preview
